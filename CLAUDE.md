...
- **인증(통합)**: 세 역할 `admin`/`booster`/`customer`를 **단일 쿠키 `xyz_session`**(`lib/session.ts`)으로 통합. 토큰은 `role:userId:expiry` HMAC-SHA256 서명(role이 서명에 포함돼 위조 불가). 서명 키 `getAuthSecret()` = `AUTH_SECRET ?? ADMIN_PASSWORD`(빈 값이면 fail-closed). 쿠키 `HttpOnly; SameSite=Strict`(prod `Secure`).
  ...
  - **현재 티어 자동 조회 = op.gg 파싱**(`getSoloTier`, `/api/riot/tier`): 소환사 페이지를 **GET**해 서버 렌더 설명문(`... current SOLORANKED rank is <tier> Division <n> <lp> LP ...`)을 정규식으로 파싱 → `{ranked,tierIndex,division,lp,level}`. **소환사 레벨**은 프로필 아이콘 배지(`class="mt-[-11px] text-center"><span>NNN</span>`)에서 파싱(설명문·RSC엔 없음, 이 배지가 유일한 소스 — op.gg가 클래스 바꾸면 깨질 수 있어 실패 시 `level:null`). op.gg 내부 API(`lol-web-api.op.gg`)는 서버에서 도달 불가(HTTP 000)라 못 씀. HEAD(실존)와 달리 본문 필요. 언랭·미존재는 `ranked:false`(에러 아님, level은 있을 수 있음), 차단/장애는 503. **키 불필요**. 공개 엔드포인트라 세션 게이트 없음. tierIndex 0(아이언)~9(챌린저)는 `components/quote/constants.ts`의 `TIERS` 인덱스와 일치.
  - **견적 계산기 1단계 = 롤 닉네임**(`components/quote/QuoteCalculator.tsx`): 로그인 시 저장된 롤 닉네임(`/api/account/nicknames` GET) 중 선택, 비로그인 시 직접 입력 → "확인"이 `/api/riot/tier`로 현재 티어·레벨을 받아 `currentTier`/`currentDivision`(+ lp→`currentLp` 버킷)에 반영하고 **티어 정보 카드로 표시**(자동으로 안 넘어감 — 사용자가 "다음"으로 진행). **30레벨 미만이면 랭크 불가라 서비스는 일반 게임만 노출**(`under30`). 서비스 선택은 2단계. **수동 "현재 랭크" 단계는 제거됨**(티어는 닉네임 조회로만 채운다) — 수량 입력이 필요한 서비스만 해당 단계를 수량 입력으로 노출하고(저티어 보장제는 없음), 조회 실패·언랭은 카카오 상담으로 안내. 고티어 점수 보장제/저티어 티어 보장제는 티어 제한이 있어 서비스 선택 시 `selectService`가 범위 밖 티어를 보정(조회값이 덮일 수 있음).
