  ...
  const saveNotice = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingId) {
        setNotices((cur) =>
          cur
            .map((n) => (n.id === editingId ? data.notice : n))
            .sort(
              (a, b) =>
            ...
            <form onSubmit={saveNotice} className="grid gap-3">
              ...
              <label className="grid gap-2">
                ...
                <textarea
                  ...
                  maxLength={3000}
                  rows={4}
                  className="resize-none rounded-2xl border border-white/10 bg-black/30 px-4 py-3 leading-7 text-white outline-none transition placeholder:text-zinc-600 focus:border-gold/50 w-full"
          ...
          <div className="overflow-hidden rounded-[30px] border border-gold/15 bg-white/[.035]">
            ...
            {notices.map((notice) => (
              <button
                ...
                className={`grid w-full gap-2 border-b border-white/8 p-4 text-left transition last:border-b-0 hover:bg-white/4 ${selectedNotice?.id === notice.id ? "bg-white/4.5" : ""}`}
              >
            ...
            <article className="rounded-[30px] border border-gold/15 bg-white/[.035] p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex shrink-0 gap-2">
                  <button
                    type="button"
                    onClick={() => void duplicateNotice(selectedNotice)}
                    className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-zinc-400 transition hover:border-gold/40 hover:text-white"
                    aria-label="공지 복제"
                  >
                    <Copy size={17} />
