...
import { Copy, Loader2, LogOut, Pencil, Pin, Plus, Trash2, X } from "lucide-react";
...
export default function AdminNoticeBoard({
  ...
  const duplicateNotice = async (notice: Notice) => {
    try {
      const response = await fetch("/api/notices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: `${notice.title} (복사본)`, content: notice.content, image: notice.image, pinned: false }),
      });
      const data = (await response.json()) as NoticeResponse;
    ...
    <>
      {modalOpen && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget && mousedownOnOverlay.current) closeModal(); }}
        >
          <div className="w-full max-w-xl rounded-[34px] border border-gold/20 bg-[#111] p-6 shadow-2xl sm:p-8 max-h-[90dvh] overflow-y-auto sm:max-h-[95dvh]">
            <div className="mb-4 flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={closeModal} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-zinc-400 transition hover:border-gold/40 hover:text-white" aria-label="닫기">
                <X size={18} />
              </button>
            </div>

            <form onSubmit={saveNotice} className="grid gap-3">
              ...
              <label className="grid gap-2">
                ...
                <textarea value={form.content} onChange={(e) => setForm((f) => ({ ...f, content: e.target.value }))} maxLength={3000} rows={4} className="resize-none rounded-2xl border border-white/10 bg-black/30 px-4 py-3 leading-7 text-white outline-none transition placeholder:text-zinc-600 focus:border-gold/50 w-full" placeholder="공지 내용을 입력해주세요." />
              </label>
              ...
              <div className="grid gap-2">
                ...
                {currentImage ? (
                  ...
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-white/15 bg-white/3 px-4 py-4 text-sm font-bold text-zinc-400 transition hover:border-gold/40 hover:text-white">
      ...
      <div className="space-y-5">
        <div className="flex items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={openWrite} className="inline-flex items-center gap-2 rounded-full bg-gold-gradient px-5 py-3 text-sm font-black text-black shadow-gold-sm transition">
              <Plus size={16} />공지 작성
            </button>
            <button type="button" onClick={() => void logout()} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-3 text-sm font-bold text-zinc-400 transition hover:border-gold/40 hover:text-white">
              <LogOut size={16} />로그아웃
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div className="overflow-hidden rounded-[30px] border border-gold/15 bg-white/[.035]">
            {notices.length === 0 && (
              <p className="p-6 text-sm text-zinc-500">등록된 공지사항이 없습니다.</p>
            )}
            {notices.map((notice) => (
              <button key={notice.id} type="button" onClick={() => setSelectedId(notice.id)} className={`grid w-full gap-2 border-b border-white/8 p-4 text-left transition last:border-b-0 hover:bg-white/4 ${selectedNotice?.id === notice.id ? "bg-white/4.5" : ""}`}>
            ...
            <article className="rounded-[30px] border border-gold/15 bg-white/[.035] p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex shrink-0 gap-2">
                  <button type="button" onClick={() => void duplicateNotice(selectedNotice)} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-zinc-400 transition hover:border-gold/40 hover:text-white" aria-label="공지 복제"><Copy size={17} /></button>
                  <button type="button" onClick={() => openEdit(selectedNotice)} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-zinc-400 transition hover:border-gold/40 hover:text-white" aria-label="공지 수정"><Pencil size={17} /></button>
